...
import 'package:phoenix_club/pricing/webservices/pricing_webervice.dart';
...
class _StatusBarPaddingSliver extends SingleChildRenderObjectWidget {
  ...
  void updateRenderObject(BuildContext context,
      _RenderStatusBarPaddingSliver renderObject) {
    renderObject
      ..maxHeight = maxHeight
      ..scrollFactor = scrollFactor;
  }
...
class _AllSectionsView extends AnimatedWidget {
  ...
  Widget _build(BuildContext context, BoxConstraints constraints) {
    ...
    translation:
    Alignment
    ((selectedIndex.value - sectionIndex) * 2.0
    - 1.0, -1.0),
...
class _PricingState extends State<PricingScreen> {
  ...
  final _webService = PricingWebService();
  ...
  @override
  void initState() {
    _webService.getList().then((list) async {
      if (list != null) {
        allSections.clear();
        for (int i = 0; i < list.length; i++) {
          List<SectionDetail> details = List();
          final _list = await _webService.getType(list[i].id);
          for (int j = 0; j < _list.length; j++) {
            MembershipTypeEntity entity = _list[j];
            details.add(SectionDetail(
                title: Strings.PERIODE, subtitle: entity.periode));
            details.add(SectionDetail(
                title: Strings.SC_NUMBER,
                subtitle: entity.scNumber.toString()));
            details.add(SectionDetail(
                title: Strings.PRICE, subtitle: entity.price.toString()));
            if (_list.length > j + 1)
              details.add(SectionDetail(title: "", subtitle: ""));
          }

          allSections.add(Section(
            title: list[i].label,
            details: details,
            backgroundAsset: list[i].photo
          ));
        }
      } else
